    ...
    <section id="início" className="relative min-h-screen flex items-center pt-20 overflow-hidden">
      ...
      <div className='max-w-[1280px] mx-auto w-full relative'>
        <div className="container-section mx-0 z-10 text-white">
          ...
          {/* Dashboard visualization - visible on all screen sizes */}
          <div className={`${isMobile ? 'mt-12 w-full' : 'absolute right-0 top-0 w-1/2'} opacity-0 animate-fade-in ${isMobile ? 'animate-delay-300' : 'animate-fade-in-right animate-delay-400'} ${isMobile ? 'block' : 'hidden lg:block'}`}>
            <div className={`relative ${isMobile ? '' : 'ml-20'}`}>
              <div className={`absolute ${isMobile ? '-top-10 -left-10 w-20 h-20' : '-top-20 -left-20 w-40 h-40'} bg-white/10 rounded-full blur-xl`}></div>
              <div className={`absolute ${isMobile ? '-bottom-5 -right-5 w-30 h-30' : '-bottom-10 -right-10 w-60 h-60'} bg-white/5 rounded-full blur-xl`}></div>
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-6 border border-white/20">
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white/20 rounded p-4">
                    <div className="h-4 w-16 bg-white/30 rounded mb-2"></div>
                    <div className="h-8 w-22 bg-white/40 rounded-lg"></div>
                  </div>
                  <div className="bg-white/20 rounded p-4">
                    <div className="h-4 w-16 bg-white/30 rounded mb-2"></div>
                    <div className="h-8 w-22 bg-white/40 rounded-lg"></div>
                ...
                <div className={`mt-4 ${isMobile ? 'h-24' : 'h-32'} bg-white/20 rounded-lg overflow-hidden`}>
